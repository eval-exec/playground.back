...
fn build_context(now: &Instant) -> Context {
    let p0 = thread::spawn(|| {
        let mm0 = parse_info_level_log(
            "/home/exec/Projects/github.com/nervosnetwork/ckb-run-log/ckb-main/data/logs/run.log",
        );
        mm0
    });
    let p1 = thread::spawn(|| {
        let mm1 = parse_info_level_log(
            "/home/exec/Projects/github.com/nervosnetwork/ckb-yamux-1M/data/logs/run.log",
        );
        mm1
    });
    let e0 = thread::spawn(|| {
...
impl Context {
    ...
    fn draw_time_cost(&self) {
        ...
        let points = points.iter().map(|(k, v)| (*k - first, *v)).collect();
...
fn draw_u64(
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(data0.first().unwrap().0..data0.last().unwrap().0, 0..y_max)?;
    chart
